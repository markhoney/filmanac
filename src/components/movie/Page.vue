<template>
	<div>
		<section class="h-64 bg-cover bg-center mb-8" :style="{'background-image': `url('${movie.images && movie.images.fanart && movie.images.fanart.path && movie.images.fanart.path.src}')`}">
			<div style="backdrop-filter: blur(5px) brightness(50%)" class="flex w-full h-full">
				...
				<div style="backdrop-filter: blur(1px) brightness(50%)" class="flex-auto ml-8">
					...
					<Rating class="mt-2" :percent="movie.score" :stars="5" :numeric="true" />
				</div>
			</div>
		</section>
		<section class="mx-64 pa-16 text-xl">
			...
			<ul>
				<li v-for="event in movie.events" :key="event.id"><g-link :to="event.dayofyear.path">{{[event.month.title, event.day.id + event.day.ordinal, event.year && event.year.id, '-', event.title].join(' ')}}</g-link></li>
			</ul>
		</section>
	</div>
</template>
